...
function createBlueAbstractSVG(title, category) {
...
async function fixMissingBase64() {
  console.log("=== Checking Supabase blogs table for missing Base64 images ===");
  ...
  for (const blog of blogs) {
    if (!blog.featured_image_base64 || blog.featured_image_base64.length < 50) {
      console.log(`Generating Blue-themed Base64 image for: "${blog.title}"...`);
      const svg = createBlueAbstractSVG(blog.title, blog.category || "AI Engineering");
      ...
      const { error: updateErr } = await supabase
        .from("blogs")
        .update({
          featured_image_base64: base64,
          content: updatedContent,
        })
        .eq("slug", blog.slug);

      if (updateErr) {
        console.error(`❌ Update failed for "${blog.slug}":`, updateErr.message);
      } else {
        console.log(`✅ Fixed Base64 image for "${blog.title}" (${blog.slug})!`);
      }
    } else {
      console.log(`✅ "${blog.title}" already has valid Base64 image (Length: ${blog.featured_image_base64.length} chars).`);
    }
  }
}
