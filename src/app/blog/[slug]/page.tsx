...
// Read blog JSON file safely
function getBlogData(slug: string) {
  const filePath = path.join(process.cwd(), "content", "blogs", slug, "content.json");
  if (!fs.existsSync(filePath)) return null;
  try {
    const raw = fs.readFileSync(filePath, "utf8");
    return JSON.parse(raw);
  } catch (err) {
    console.error(`Failed to read content.json for slug: ${slug}`, err);
    return null;
  }
}
...
export async function generateStaticParams() {
  ...
  try {
    ...
    console.error("Failed to read static directory folders for blogs:", err);
    return [];
  }
}
...
export async function generateMetadata({ params }: BlogPageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      images: [
        {
          url: `${baseUrl}${blog.featuredImage}`,
          width: 1200,
          height: 630,
          alt: blog.title,
        },
    ...
    twitter: {
      ...
      title: blog.metaTitle || blog.title,
      description: blog.metaDescription || blog.excerpt,
      images: [`${baseUrl}${blog.featuredImage}`],
    },
  };
}
...
export default async function BlogDetailPage({ params }: BlogPageProps) {
  ...
  const jsonLd = {
    ...
    "@type": "BlogPosting",
    "headline": blog.title,
    "image": `https://www.skillsha.com${blog.featuredImage}`,
      ...
      <main className="pt-28 md:pt-36 min-h-screen bg-[#FAF9F6] dark:bg-[#050505] transition-colors duration-300">
        <article className="max-w-[800px] mx-auto px-4 md:px-6 pb-20">
          ...
          <div className="flex items-center gap-3 mb-5 select-none">
            <span className="text-[10px] font-bold uppercase tracking-wider text-brand-orange bg-brand-orange/10 px-3 py-1 rounded-full border border-brand-orange/10">
          ...
          {/* Excerpt intro */}
          <p className="text-[16px] md:text-[18px] text-zinc-550 dark:text-[#9CA3AF] leading-relaxed mb-8 font-medium italic border-l-2 border-brand-orange pl-4">
            {blog.excerpt}
          </p>

          {/* Author/Date Info */}
          <div className="flex items-center gap-3 border-y border-zinc-200 dark:border-white/5 py-4 mb-8">
            <div className="w-9 h-9 rounded-full bg-brand-orange/10 flex items-center justify-center font-bold text-brand-orange text-sm select-none">
              SW
            </div>
            <div>
          ...
          <div className="prose dark:prose-invert max-w-none text-[15px] md:text-[16px] text-zinc-850 dark:text-zinc-300 leading-relaxed font-sans space-y-6">
            ...
            {blog.sections && blog.sections.map((sec: any, sIdx: number) => (
              <div key={sIdx} className="pt-6 space-y-3.5">
                ...
                <div 
                  className="whitespace-pre-wrap prose-li:list-disc prose-tables:border prose-a:text-brand-orange hover:prose-a:underline" 
            ...
            {/* Call to Action card */}
            {blog.ctaText && (
              <div className="relative group overflow-hidden rounded-3xl p-6 md:p-8 bg-zinc-900 dark:bg-zinc-950 border border-brand-orange/30 text-white mt-12 shadow-xl">
                <div className="absolute top-0 right-0 w-40 h-40 bg-brand-orange/10 blur-3xl rounded-full" />
                ...
                <a
                  href="/courses"
                  className="inline-flex items-center justify-center px-6 py-3 bg-brand-orange text-white text-[12px] font-bold uppercase tracking-wider rounded-xl shadow-lg shadow-brand-orange/10 hover:scale-[1.02] active:scale-95 transition-all select-none relative z-10 cursor-pointer"
                >
                  {blog.ctaText}
                </a>
              </div>
          ...
          {/* FAQs Accordion Accorded section */}
